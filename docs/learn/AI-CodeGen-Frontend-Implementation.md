...
- **数据解析**: 后端返回的是 `data: {"d": "内容"}`。前端需解析 JSON 并提取 `d` 字段进行拼接。
...
**要点**：
- **URL 构造**: `http://localhost:8700/api/static/{codeGenType}_{appId}/index.html`。
...
const updatePreview = () => {
  ...
  // 拼接预览地址，增加 t 参数防止浏览器加载缓存的 index.html
  iframeUrl.value = `http://localhost:8700/api/static/${codeGenType}_${appId}/index.html?t=${Date.now()}`
}
```
...
const updatePreview = () => {
  const codeGenType = app.value?.codeGenType || 'REACT'
  // 拼接预览地址，增加 t 参数防止浏览器加载缓存的 index.html
  iframeUrl.value = `http://localhost:8123/api/static/${codeGenType}_${appId}/index.html?t=${Date.now()}`
}
```
